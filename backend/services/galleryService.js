...
export const galleryService = {
  ...
  async getImageById(id) {
    const query = 'SELECT * FROM gallery_images WHERE id = ? AND is_active = TRUE';
  ...
  async deleteImage(id) {
    const query = 'UPDATE gallery_images SET is_active = FALSE WHERE id = ?';
    return await executeQuery(query, [id]);
  },
  ...
  async searchImagesByTags(tags) {
    const query = `
      SELECT * FROM gallery_images 
      WHERE is_active = TRUE 
      AND JSON_SEARCH(metadata, 'one', ?, NULL, '$.tags') IS NOT NULL
      ORDER BY display_order, created_at
    `;
    return await executeQuery(query, [tags]);
  },

  // Update display order
  async updateDisplayOrder(images) {
    const queries = images.map((image, index) => ({
      query: 'UPDATE gallery_images SET display_order = ? WHERE id = ?',
      params: [index + 1, image.id]
    }));
